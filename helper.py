...
def predict(test_data_path, results_path, model):
    ...
    for name in os.listdir(test_data_path):
        ...
        prediction, aleatoric, epistemic = predict_mc(model, image, T=5)  # change to T=50 for precise uncertainty maps
        ...
        ax.axis('off')
        plt.tight_layout()
        fig.savefig(results_path + 'uncertainty/aleatoric_uncertainty_{}'.format(name), bbox_inches='tight',
        ...
        ax.axis('off')
        plt.tight_layout()
        fig.savefig(results_path + 'uncertainty/epistmeic_uncertainty_{}'.format(name), bbox_inches='tight',
                    transparent=True, dpi=500)
